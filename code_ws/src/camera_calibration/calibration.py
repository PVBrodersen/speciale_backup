...
def main():
    ...
    parser.add_argument('-i', '--input', metavar='', type=lambda p: Path(p).absolute(), help='the input directory', default=Path(__file__).absolute().parent / "input/img")
    parser.add_argument('-c', '--csv', metavar='', type=lambda p: Path(p).absolute(), help='the csv input directory', default=Path(__file__).absolute().parent / "input/csv")
    ...
    parser.add_argument('--min_coverage', metavar='', type=float, default=1, help='Minimum percentage coverage of the checkerboard in the image for image to be considered (default 25)')
    ...
    parser.add_argument('--distance_threshold', metavar='', type=float, default=0.2, help='Threshold for acceptable deviation between espected location and actual location of points when assigning real world coordinates (default 0.1)')
    ...
    list_img_input = generate_list_of_images(args.input)
    if len(list_img_input) == 0:
        print("ERROR: No files found at the provided input location, program stopped")
        exit()
    print(list_img_input)
    list_csv_input = generate_list_of_csv(args.csv)
    if len(list_csv_input) == 0: 
        print("ERROR: No files found at the provided csv input location, program stopped")
        exit()
    print(list_csv_input)
    list_input = zip(list_img_input,list_csv_input)
    # detect corners in every image
    for file_path, csv_path in list_input:
        (objp, imgp, coverage, statistics) = detect_calibration_pattern_in_image(file_path, csv_path, args.output, args.debug, 
    ...
    # calibrate camera
    img = cv2.imread(str(list_img_input[0])) 
    ...
    stats_after = undistort_images(list_img_input, path_to_undistorted_images, matrix, distortion, args.fisheye)
    # print output to screen and file
    print_output(matrix, distortion, args.fisheye)
    write_output(list_img_input, args.output, matrix, distortion, coverage_images, stats_before, stats_after, args.min_coverage, args.fisheye)
    write_calibration(list_img_input, args.output, matrix, distortion, args.fisheye)
    if args.debug:
        ...
        show_estimation_distortion(list_img_input, all_imgpoints, matrix, distortion, args.fisheye, path_to_estimation_distortion, args.scaling_debug)
...
def detect_calibration_pattern_in_image(file_path, csv_path, output_folder, debug, 
    ...
    # Get contours from csv 
    centers = []
    with open(csv_path,'r') as csvfile:
        csvreader = csv.reader(csvfile)

        for row in csvreader:
            centers.append((int(row[1]),int(row[2])))
    center_list = np.array(centers,dtype=np.int32)

    corners, coverage, statistics = cbcd.detect_chess_board_corners(img, center_list, debug, path_to_image=file_path, path_to_output_folder=output_folder)
    objp = []
    imgp = []
