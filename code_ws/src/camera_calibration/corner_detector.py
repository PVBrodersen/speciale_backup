...
class ChessBoardCornerDetector:
    ...
    def detect_chess_board_corners(self, img, center_list, debug=False, *, path_to_image=None, path_to_output_folder=None):
        ...
        centers = center_list #self.locate_centers_of_peaks(contour_list)
    ...
    def make_statistics(self, contour_list):
        ...
        # relative_responses_thresholded = self.threshold_responses(response_relative_to_neighbourhood)
        # Locate centers of peaks
        centers = self.locate_centers_of_peaks(contour_list)
        # Select central center of mass
        selected_center = self.select_central_peak_location(centers)
        # Enumerate detected peaks
        calibration_points = self.enumerate_peaks(centers, selected_center)
    ...
    def locate_centers_of_peaks(self, contour_list):
    ...
    def show_detected_calibration_points(self, img, calibration_points):
        ...
        for x_index, temp in calibration_points.items():
            for y_index, cal_point in temp.items():
                cv2.circle(canvas, tuple(cal_point.astype(int)), int(self.kernel_size / 2), (0, 255 * (y_index % 2), 255 * (x_index % 2)), 1)
    ...
    #     return local_min_image_temp, t1 = cv2.findContours(np.uint8(relative_responses_thresholded), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
